/*!
 * jQuery typeaheadMulti
 * Copyright (C) 2017 RunningCoder.org
 * Licensed under the MIT license
 *
 * @author Tom Bertrand
 * @version 2.10.4 (2017-10-17)
 * @link http://www.runningcoder.org/jquerytypeaheadMulti/
 */
(function (factory) {
    if (typeof define === "function" && define.amd) {
        define("jquery-typeaheadMulti", ["jquery"], function (jQuery) {
     ...
     * @private
     * Default options
     * @link http://www.runningcoder.org/jquerytypeaheadMulti/documentation/
     */
    var _options = {
        input: null,                // *RECOMMENDED*, jQuery selector to reach typeaheadMulti's input for initialization
        minLength: 2,               // Accepts 0 to search on focus, minimum character length to perform a search
        maxLength: false,           // False as "Infinity" will not put character length restriction for searching results
        maxItem: 50,                 // Accepts 0 / false as "Infinity" meaning all the results will be displayed
        dynamic: false,             // When true, typeaheadMulti will get a new dataset from the source option on every key press
        ...
        dynamicFilter: null,        // Filter the typeaheadMulti results based on dynamic value, Ex: Players based on TeamID
        backdrop: false,            // Add a backdrop behind typeaheadMulti results
        backdropOnFocus: false,     // Display the backdrop option as the typeaheadMulti input is :focused
        cache: false,               // Improved option, true OR 'localStorage' OR 'sessionStorage'
        ttl: 3600000,               // Cache time to live in ms
        compression: false,         // Requires LZString library
        searchOnFocus: false,       // Display search results on input focus
        blurOnTab: true,            // Blur typeaheadMulti when Tab key is pressed, if false Tab will go though search results
        ...
        loadingAnimation: true,     // Display a loading animation when typeaheadMulti is doing request / searching for results
        filter: true,               // Set to false or function to bypass typeaheadMulti filtering. WARNING: accent, correlativeTemplate, offset & matcher will not be interpreted
        matcher: null,              // Add an extra filtering function after the typeaheadMulti functions
        source: null,               // Source of data for typeaheadMulti to filter
        callback: {
            onInit: null,               // When typeaheadMulti is first initialized (happens only once)
            onReady: null,              // When the typeaheadMulti initial preparation is completed
            ...
            onClickBefore: null,        // Possibility to e.preventDefault() to prevent the typeaheadMulti behaviors
            onClickAfter: null,         // Happens after the default clicked behaviors has been executed
            onDropdownFilter: null,     // When the dropdownFilter is changed, trigger this callback
            onSendRequest: null,        // Gets called when the Ajax request(s) are sent
            onReceiveRequest: null,     // Gets called when the Ajax request(s) are all received
            onPopulateSource: null,     // Perform operation on the source data before it gets in typeaheadMulti data
            onCacheSave: null,          // Perform operation on the source data before it gets in typeaheadMulti cache
            onSubmit: null,             // When typeaheadMulti form is submitted
            onCancel: null              // Triggered if the typeaheadMulti had text inside and is cleared
        },
        selector: {
            container: "typeaheadMulti__container",
            result: "typeaheadMulti__result",
            list: "typeaheadMulti__list filter-scroll",
            group: "typeaheadMulti__group",
            item: "typeaheadMulti__item",
            empty: "typeaheadMulti__empty",
            display: "typeaheadMulti__display",
            query: "typeaheadMulti__query",
            filter: "typeaheadMulti__filter",
            filterButton: "typeaheadMulti__filter-button",
            dropdown: "typeaheadMulti__dropdown",
            dropdownItem: "typeaheadMulti__dropdown-item",
            labelContainer: "typeaheadMulti__label-container",
            label: "typeaheadMulti__label",
            button: "typeaheadMulti__button",
            backdrop: "typeaheadMulti__backdrop",
            hint: "typeaheadMulti__hint",
            cancelButton: "typeaheadMulti__cancel-button"
        },
    ...
    /**
     * @constructor
     * typeaheadMulti Class
     *
     * @param {object} node jQuery input object
     * @param {object} options User defined options
     */
    var typeaheadMulti = function (node, options) {
        this.rawQuery = node.val() || "";   // Unmodified input query
        this.query = node.val() || "";      // Input query
        this.selector = node[0].selector;   // typeaheadMulti instance selector (to reach from window.typeaheadMulti[SELECTOR])
        ...
        this.searchGroups = [];             // Array of groups to generate when typeaheadMulti searches data
        this.generateGroups = [];           // Array of groups to generate when typeaheadMulti requests data
        ...
        this.options = options;             // typeaheadMulti options (Merged default & user defined)
        this.node = node;                   // jQuery object of the typeaheadMulti <input>
        this.namespace =
            "." +
            this.helper.slugify.call(this, this.selector) +
            _namespace;                     // Every typeaheadMulti instance gets its own namespace for events
        this.isContentEditable = typeof this.node.attr('contenteditable') !== "undefined"
            && this.node.attr('contenteditable') !== "false";
        this.container = null;              // typeaheadMulti container, usually right after <form>
        this.resultContainer = null;        // typeaheadMulti result container (html)
    ...
    typeaheadMulti.prototype = {
        _validateCacheMethod: function (cache) {
            ...
            try {
                window[cache].setItem("typeaheadMulti", "typeaheadMulti");
                window[cache].removeItem("typeaheadMulti");
            } catch (e) {
                supported = false;
            }
                ...
                if (!groupSource.data && !groupSource.ajax) {
                    ...
                    if (this.options.debug) {
                        _debug.log({
                            ...
                            group +
                            '.[data|ajax]" is Missing - typeaheadMulti dropped'
                        });
        ...
        init: function () {
            ...
            if (this.options.debug) {
                _debug.log({
                    ...
                    message: "OK - typeaheadMulti activated on " + this.selector
                });
        ...
        delegateEvents: function () {
            ...
            this.node
                ...
                .on("reset", function () {
                    // #221 - Reset typeaheadMulti on form reset.
                    // setTimeout to re-queue the `input.typeaheadMulti` event at the end
                    setTimeout(function () {
                        scope.node.trigger("input" + scope.namespace);
                        // #243 - minLength: 0 opens the typeaheadMulti results
                        scope.hideLayout();
                    });
                });
                ...
                .on(events.join(" "), function (e, data) {
                    switch (e.type) {
                        ...
                        case "input":
                            ...
                            // #195 Trigger an onCancel event if the typeaheadMulti is cleared
            ...
            for (var i = 0, ii = this.generateGroups.length; i < ii; ++i) {
                ...
                if (cache) {
                    dataInStorage = window[cache].getItem(
                        "typeaheadMulti_" + this.selector + ":" + group
                    );
                    if (dataInStorage) {
                        if (compression) {
        ...
        populateSource: function (data, group, path) {
            ...
            if (
                cache && !window[cache].getItem("typeaheadMulti_" + this.selector + ":" + group)
            ) {
        ...
        /**
         * Key Navigation
         * tab 9: if option is enabled, blur typeaheadMulti
        ...
        navigate: function (e) {
            ...
            // #284 Blur typeaheadMulti when "Tab" key is pressed
                ...
                (function (i, item, liHtml) {
                    liHtml.on("click", function (e, originalEvent) {
                        ...
                        if (scope.options.multiselect) {
                            ...
                            scope.addMultiselectItemLayout(templateValue,summary);
                        } else {
        ...
        addMultiselectItemLayout: function (templateValue,summary) {
            var scope = this,
        ...
        adjustInputSize: function () {
            ...
            this.container
                ...
                .find("input, textarea, [contenteditable], .typeaheadMulti__hint")
                .css({
        ...
        showLayout: function () {
            ...
            function _addHtmlListeners() {
                ...
                // If typeaheadMulti is blured by pressing the "Tab" Key, hide the results
                $("html")
                ...
                // If typeaheadMulti is blured by clicking outside, hide the results
                $("html")
        ...
        helper: {
            ...
            /**
             * For [contenteditable] typeaheadMulti node only,
    ...
    /**
     * @public
     * Implement typeaheadMulti on the selected input node.
     *
     * @param {Object} options
     * @return {Object} Modified DOM element
     */
    $.fn.typeaheadMulti = $.typeaheadMulti = function (options) {
        return _api.typeaheadMulti(this, options);
    };

    /**
     * @private
     * API to handles typeaheadMulti methods via jQuery.
     */
    var _api = {
        /**
         * Enable typeaheadMulti
         *
         * @param {Object} node
         * @param {Object} options
         * @returns {*}
         */
        typeaheadMulti: function (node, options) {
            if (!options || !options.source || typeof options.source !== "object") {
                // {debug}
                _debug.log({
                    node: node.selector || (options && options.input),
                    function: "$.typeaheadMulti()",
                    arguments: JSON.stringify((options && options.source) || ""),
                    message: 'Undefined "options" or "options.source" or invalid source type - typeaheadMulti dropped'
                });
            ...
            if (typeof node === "function") {
                if (!options.input) {
                    ...
                    _debug.log({
                        ...
                        message: 'Undefined "options.input" - typeaheadMulti dropped'
                    });
            ...
            if (!node.length) {
                ...
                _debug.log({
                    ...
                    message: "Unable to find jQuery input element - typeaheadMulti dropped"
                });
            ...
            if (node.length === 1) {
                ...
                /*jshint boss:true */
                return (window.typeaheadMulti[node[0].selector] = new typeaheadMulti(node, options));
            } else {
                var instances = {},
                    instanceName;
                ...
                for (var i = 0, ii = node.length; i < ii; ++i) {
                    ...
                    window.typeaheadMulti[instanceName] = instances[instanceName] = new typeaheadMulti(node.eq(i), options);
                }
    ...
    var _debug = {
        ...
        print: function () {
            if (
                typeaheadMulti.prototype.helper.isEmpty(this.table) || !console || !console.table
            ) {
                return;
            }

            if (console.group !== undefined || console.table !== undefined) {
                console.groupCollapsed("--- jQuery typeaheadMulti Debug ---");
    ...
    _debug.log({
        message: "WARNING - You are using the DEBUG version. Use /dist/jquery.typeaheadMulti.min.js in production."
    });
